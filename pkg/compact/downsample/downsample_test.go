...
var (
	...
	realisticChkDataWithStaleMarkers = [][]sample{
		{
...
func TestDownsample(t *testing.T) {
	...
	for _, tcase := range []*downsampleTestCase{
		...
		},
		{
			...
			inRaw:      realisticChkDataWithStaleMarkers,
		...
		}(),
		{
			...
			expected: []map[AggrType][]sample{
				{
					0x0: {{t: 499, v: 29}, {t: 999, v: 100}, {t: 1499, v: 23}, {t: 1999, v: 100}},
					0x1: {{t: 499, v: 29}, {t: 999, v: 100}, {t: 1499, v: 23}, {t: 1999, v: 100}},
					0x2: {{t: 499, v: -3}, {t: 999, v: 0}, {t: 1499, v: -3}, {t: 1999, v: 0}},
					0x3: {{t: 499, v: 10}, {t: 999, v: 100}, {t: 1499, v: 10}, {t: 1999, v: 100}},
					0x4: {{t: 99, v: 100}, {t: 499, v: 210}, {t: 999, v: 320}, {t: 1499, v: 530}, {t: 1999, v: 640}, {t: 2299, v: 750}, {t: 2299, v: 110}},
				},
			},
		},
		{
		...
		// TODO(bwplotka): This is wrong, we should produce 2 chunks. Fix it https://github.com/thanos-io/thanos/issues/2542
		func() *downsampleTestCase {
...
func testDownsample(t *testing.T, d *downsampleTestCase) {
	...
	if len(d.inRaw) > 0 {
		...
		fakeMeta.Thanos.Downsample.Resolution = d.resolution - 1
		for _, chk := range d.inAggr {
			ser.chunks = append(ser.chunks, encodeTestAggrSeries(chk))
		}
		fmt.Println(ser.chunks)
	}
	mb.addSeries(ser)

	id, err := Downsample(logger, fakeMeta, mb, dir, d.resolution)
	if d.expectedDownsamplingErr != nil {
		testutil.NotOk(t, err)
		testutil.Equals(t, d.expectedDownsamplingErr(ser.chunks).Error(), err.Error())
		return
	}
	testutil.Ok(t, err)
...
func TestCounterSeriesIterator(t *testing.T) {
	...
	for _, tcase := range []struct {
		...
		},
		{
			...
			chunks: realisticChkDataWithStaleMarkers,
			expected: []sample{
